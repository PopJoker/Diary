今天		Google 上架	GusBLE 已上架
					GusEMS 已上架

		Hirack HMI	GusEMS HiRack 自動補電流程有不好的地方 今天再看程式碼的時候有看到 有改好目前整體測試沒問題

明天		Hirack Web	GusEMS HiRack 設備管理的部分會加入自動補電的功能 跟前面平衡設置一樣會用HMI訂閱訊息 然後對電池櫃下命令
