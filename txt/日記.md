今天		Google 上架	GusBLE 待回復
					GusEMS 待回復

		Hirack HMI	GusEMS HiRack 有找到在HMI接到訊息但沒有發出平衡訊號的原因 處理完後就可以正常下指令了 待副理更新完韌體後 同步更新

明天		Hirack HMI	會優化整合一下HMI目前的指令 看有沒有巨集命令可以簡化 並且優化一下 Hirack Webems的控制介面
